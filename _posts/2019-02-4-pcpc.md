...
|Lecture 3 | 12/04| Introduction to Amazon Web Services 1 - 2 | [pdf](https://spagnuolocarmine.github.io/assets/files/aws-overview.pdf) | 
|Lecture 4 | 16/04| Introduction to Amazon Web Services 3 - 4 | [pdf](https://spagnuolocarmine.github.io/assets/files/aws-overview.pdf) | 
|Lecture 5 | 18/04| Amazon Web Service Practice|  | 
|Lecture 6 | 30/04| Model and Architecture of Parallel Computation + Metrics of Parallel Algorithm | B2: C2.1-C2.2-C5.1-C5.3-C5.4 C3.1-C3.2| 
|Lecture 7 | 02/05| Algorithms for shared-memory  |  B2: C3.5.1-C3.5.4-C3.5.5| 
|Lecture 8 | 03/05| Design of Parallel Algorithm + Message-passing Programming 1 | [counting sort](https://spagnuolocarmine.github.io/news/counting-sort/) | 
|Lab 1     | 07/05| Message-passing Programming |  | 
|Lecture 9 | 09/05| Message-passing Programming |  | 
|Lab 2     | 10/05| Amazon Web Service  |  | 
|Lab 3     | 14/05| Message-passing Programming |  | 
|Lecture 10| 16/05| Shared-memory Programming |  | 
|Lab 4     | 17/05| Shared-memory Programming |  | 
|Lecture 11| 21/05| Shared-memory Programming |  | 
|Lab 5     | 23/05| Shared-memory Programming |  | 
|Lecture 12| 24/05| Message-passing Shared-memory Programming |  | 
|Project| 28/05| Project Presentation |  | 
|Lecture 14| 30/05| Introdution to FLY |  | 
...
We use issues on [GitHub](https://github.com/spagnuolocarmine/spagnuolocarmine.github.io/issues). Please use it for sharing ideas solutions and issues on both theoric and pratical course part.
...
Each student must develop a solution for the assigned homework using Hybrid parallel programming approach, exploiting shared and distributed memory. The solution must be written in C programming language exploiting OpenMPI and OpenMP, using point-to-point and/or collective communication and all OpenMP features. The developed solution must be experimented on an homogeneous cluster scenario. 

The cluster must be created on the Amazon Web Services using a supported [AWS Educate Instances](https://s3.amazonaws.com/awseducate-starter-account-services/AWS_Educate_Starter_Accounts_and_AWS_Services.pdf). The obtained results must be presented in terms of [strong and weak scalability](https://www.sharcnet.ca/help/index.php/Measuring_Parallel_Scaling_Performance), varying the number of computing processors from _1_ to _NUMBER-OF-VCPU-FOR-INSTANCE-TYPE_ _x_ _NUMBER-OF-INSTANCES_. For instance if we run a cluster machine of _4_ t2.large (2  nodes, we experimente the scalability of our solution for P={1,2,4,5,6,7,8}.
 

The total number of processors is the equal to the number of Virtual CPU on the running instances. The benchmark must exploiting **8** instances for the bigger experiment. The student must describe the solution and the benchmark in a README file, written in Markdown, and included in the submission. The README file must describe also the compilation phase and how to reproduce the results obtained.
...
**Example: Alice Wonderland 01/01/1865, your assignment is _alicewonderland01011865_, ```md5(_alicewonderland01011865_)=89820e2ce860966ccd9165e4029035e7```, that is solve the _pi_ problem on _t2.2xlarge_ instances.
...
2. Start two new t2.micro instances using the StarCluster AMI (ami-52a0c53b) and select the _kcluster_ key-pair, additionally rename it as _master_ and _slave0_. Set a new rule to the default security group that allows to open all TCP ports (0-65535) from anywhere (you can do it in the Configure Security Group tab).
