...
- 📅 23/03/2020
    ...
    - 🐳 [Docker Ubuntu OpenMPI](https://hub.docker.com/r/spagnuolocarmine/docker-mpi)
        - Usage: ```docker run -it --mount src="$(pwd)",target=/home,type=bind  spagnuolocarmine/docker-mpi:latest```, executes the docker container, which mounts the current path in the ``/home`` folder of the container.
